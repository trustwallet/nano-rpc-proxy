...
export interface TokenSettings {
    ...
    // time to wait for each check for receivable Nano
    receivable_interval: number
...
export function readTokenSettings(settingsPath: string): TokenSettings {
    const defaultSettings: TokenSettings = {
        ...
        token_price: 0.0001,
        payment_timeout: 180,
        receivable_interval: 2,
        receivable_threshold: "100000000000000000000000",
        receivable_count: 10,
...
export function tokenLogSettings(logger: (...data: any[]) => void, settings: TokenSettings) {
    ...
    logger("Receivable Interval: " + settings.receivable_interval)
    logger("Receivable Threshold: " + settings.receivable_threshold)
    logger("Receivable Max Count: " + settings.receivable_count)
